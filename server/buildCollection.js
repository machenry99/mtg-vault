...
const csvPath = path.join(__dirname, '../data/cards_2026_Mar_18_12-24(1).csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  const edition   = clean[7] || '';

  if (!scryfallId) continue;

  cards.push({ name, condition, foil, language, quantity, scryfallId, edition });
}
